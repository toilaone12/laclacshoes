<?php 
  ...
  $date=getdate();
  $thang= $date['year'].'-'.($date['mon'] < 10 ? '0'.$date['mon'] : $date['mon']);
  $lastDateOfMonth = date('Y-m-t');
  $nam=$date['year'];
  // var_dump($lastDateOfMonth); die;
  $sql_dtt="SELECT * FROM hoadon WHERE TinhTrang='hoàn thành' and NgayGiao BETWEEN '".$thang."-01' AND '".$lastDateOfMonth."'";
