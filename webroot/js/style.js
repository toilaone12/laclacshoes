    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        if (response.res == "success") {
          ...
          var total = (parseInt(subtotal) - parseInt(sotien1)).toLocaleString(
            "vi-VN",
...
$(document).ready(function () {
  ...
  function htmlDetail(data) {
    ...
    html += `</td>`;
    html += `<td class="align-middle">${
      bill.PhuongThucThanhToan == 1
        ? "Thanh toán khi nhận hàng"
        : bill.PhuongThucThanhToan == 2
        ? "Thanh toán bằng MoMo"
        : "Thanh toán bằng VNPAY"
    }</td>`;
    html += `<td class="align-middle">${bill.TongTien}</td>`;
    html += `<td class="align-middle"><span class="badge ${
      bill.TinhTrang.toLowerCase() == "hoàn thành"
        ? "badge-success"
        : bill.TinhTrang.toLowerCase() == "hủy bỏ"
        ? "badge-danger"
        : "badge-warning"
    } px-3 py-2 fs-13 rounded-pill text-white d-inline">${
      bill.TinhTrang.charAt(0).toUpperCase() + bill.TinhTrang.slice(1)
    }</span></td>`;
    html += `<td width="150" class="align-middle">${bill.NgayDat}</td>`;
    html += `<td width="150" class="align-middle">${
      bill.NgayGiao ? bill.NgayGiao : "Không có"
    }</td>`;
    html += `</tr>`;
    html += `</tbody>`;
    html += `</table>`;
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        if (data.res == "success") {
          ...
          $('input[name="tongtien"]').val(
            formatNumberWithCommas(subTotal - feeDiscount)
          );
        }
      },
    });
  ...
  $("#cancelBill").on("submit", function (e) {
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        // $('.list-detail').html(html);
        console.log(data);
        if (data.res == "success") alert("Hủy đơn hàng thành công");
        location.reload();
      },
    });
  });
...
L.marker([10.762622, 106.660172]).addTo(map).bindPopup("Lac Lac shoes ");
